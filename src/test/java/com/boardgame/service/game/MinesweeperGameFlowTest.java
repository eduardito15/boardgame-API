...
import com.boardgame.dto.CreateMinesweeperGameRequestDTO;
import com.boardgame.exceptions.InvalidActionException;
import com.boardgame.model.*;
import com.boardgame.service.impl.GameFlowImpl;
...
@SpringBootTest
public class MinesweeperGameFlowTest {

    private GameFlowImpl gameFlow;
    ...
    @Before
    public void setUp() {
        gameFlow = new GameFlowImpl(gameFactory, gameRepository);

        Mockito.when(gameRepository.save(any()))
                .thenReturn(createTestMockGame());

        Mockito.when(gameRepository.findById(any()))
                .thenReturn(Optional.of(createTestMockGame()));
    ...
    public void testCreateNewGame() {
        GameResponseDTO newGame = gameFlow.createNewGame(createGameRequestDTO());

        Assert.assertEquals("Not expected game id", "GID-TEST", newGame.getGameId());
        Assert.assertEquals("Not expected game name", "minesweeper", newGame.getGameName());
        Assert.assertEquals("Not expected game status", "PLAYING", newGame.getStatus().getStatus().name());
        Assert.assertFalse("Not expected ended value", newGame.getStatus().isEnded());
        Assert.assertTrue(!(newGame.getBoard().getSquare(0,0) instanceof MinesweeperSquare));
        Assert.assertEquals("Not expected time min", 0, newGame.getTimeMin());
        Assert.assertEquals("Not expected time sec", 0, newGame.getTimeSec());
    }

    @Test()
    public void invalidRowsCreateNewGameTest() {
        exceptionRule.expect(InvalidActionException.class);
        exceptionRule.expectMessage("Rows and columns must be grater than 0");

        CreateGameRequestDTO gameRequestDTO = createGameRequestDTO();
        gameRequestDTO.setRows(0);
        gameFlow.createNewGame(gameRequestDTO);
    }

    @Test()
    public void invalidColumnsCreateNewGameTest() {
        exceptionRule.expect(InvalidActionException.class);
        exceptionRule.expectMessage("Rows and columns must be grater than 0");

        CreateGameRequestDTO gameRequestDTO = createGameRequestDTO();
        gameRequestDTO.setColumns(0);
        gameFlow.createNewGame(gameRequestDTO);
    }

    @Test()
    public void invalidUsernameCreateNewGameTest() {
        exceptionRule.expect(InvalidActionException.class);
        exceptionRule.expectMessage("User name is mandatory");

        CreateGameRequestDTO gameRequestDTO = createGameRequestDTO();
        gameRequestDTO.setUser("");
        gameFlow.createNewGame(gameRequestDTO);
    }

    @Test()
    public void invalidMinesCreateNewGameTest() {
        exceptionRule.expect(InvalidActionException.class);
        exceptionRule.expectMessage("Invalid create game request");

        CreateMinesweeperGameRequestDTO gameRequestDTO = (CreateMinesweeperGameRequestDTO)createGameRequestDTO();
        gameRequestDTO.setMines(0);
        gameFlow.createNewGame(gameRequestDTO);
    }

    @Test
    public void testLoadGame() {
        GameResponseDTO newGame = gameFlow.loadGame("GID-TEST");
        ...
        Assert.assertFalse("Not expected ended value", newGame.getStatus().isEnded());
        Assert.assertTrue((newGame.getBoard().getSquare(0,0) instanceof MinesweeperSquare));
        Assert.assertTrue((newGame.getBoard().getSquare(0,1) instanceof MinesweeperSquare));
        Assert.assertTrue(!(newGame.getBoard().getSquare(0,2) instanceof MinesweeperSquare));
        Assert.assertEquals("Not expected time min", 0, newGame.getTimeMin());
        Assert.assertEquals("Not expected time sec", 0, newGame.getTimeSec());
    }

    private CreateGameRequestDTO createGameRequestDTO() {
        CreateMinesweeperGameRequestDTO requestDTO = new CreateMinesweeperGameRequestDTO();
        requestDTO.setGameName("minesweeper");
        requestDTO.setRows(8);
        requestDTO.setColumns(8);
        requestDTO.setMines(10);
        requestDTO.setUser("test");
        return  requestDTO;
    }
    ...
    private Board getGameTestBoard() {
        ...
        board.updateSquare(7,0, createMinesweeperSquare(7,0, true, false, 0, false, false));
